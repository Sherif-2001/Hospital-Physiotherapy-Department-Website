...
def editprofile():
    if 'user_patient' in session or 'user_doctor' in session and 'loggedIn' in session :  
        ...
        if 'loggedIn' in session and 'user_patient' in session and request.method == "GET" :
            ...
            sql1 = "UPDATE users SET email = %s, password= %s, type= %s WHERE email = %s"
            val1 = (email,password,'doctor',email)
            mycursor.execute(sql1, val1)
            mydb.commit()
...
def addpatient():
    if request.method == 'POST': ##check if there is post data
        ...
        if emailExist and ssnExist and emailExistAsDoctor and ssnExistAsDoctor  :
            return render_template('addpatient.html', emailExisits = True , ssnExisits=True , emailExistAsDoctor=True , ssnExistAsDoctor=True)
        elif emailExist or ssnExist or emailExistAsDoctor or ssnExistAsDoctor :
            if emailExist :
                return render_template('addpatient.html', emailExisits = True , ssnExisits=False , emailExistAsDoctor=False , ssnExistAsDoctor=False)
            elif ssnExist :
                return render_template('addpatient.html', emailExisits = False , ssnExisits=True, emailExistAsDoctor=False , ssnExistAsDoctor=False)
            elif ssnExistAsDoctor:
                return render_template('addpatient.html', emailExisits = False , ssnExisits=False, emailExistAsDoctor=True , ssnExistAsDoctor=False)
            else:    
                return render_template('addpatient.html', emailExisits = False , ssnExisits=False, emailExistAsDoctor=False , ssnExistAsDoctor=True)

        elif not re.match(r'[^@]+@[^@]+\.[^@]+', email):
            return render_template('addpatient.html', emailExisits = False , emailInvalid=True ,emailExistAsDoctor=False , ssnExistAsDoctor=False)        
        else:    
